...
void OutputManager::write_state(double time, int step, Vec X, int num_species) {
    // Collective operation: Gather data to all (or root)
    // We use VecScatterCreateToAll which gathers to all, but we only write on Rank 0.
    // Ideally use VecScatterCreateToZero if available or manual gather, but ToAll is easier with existing code.
...
void OutputManager::read_state(const std::string& filename, int step, Vec X) {
    ...
    // Create sequential vector for reading on all ranks (needed for scatter)
    Vec X_seq;
    VecScatter ctx;
    VecScatterCreateToAll(X, &ctx, &X_seq);
    ...
    if (rank == 0) {
        ...
        if (fid < 0) {
            // Can't easily throw here without hanging others waiting on Scatter
            // Ideally broadcast error. For now, just print and continue (will likely segfault or produce bad data)
            std::cerr << "Cannot open restart file: " << filename << std::endl;
        } else {
            std::string path = "data/step_" + std::to_string(step);
            hid_t group = H5Gopen(fid, path.c_str(), H5P_DEFAULT);
            if (group < 0) {
                H5Fclose(fid);
                std::cerr << "Step not found in restart file" << std::endl;
            } else {
    ...
    // Scatter from sequential to parallel (Rank 0 -> All)
    // Note: If Rank 0 failed to read, X_seq has garbage, which gets scattered.
